# This script detects tables in PDFs using Camelot and exports each table as an image.
# Camelot provides better table detection and more accurate bounding boxes than PyMuPDF.
# Designed for programmatic use in data extraction pipelines.
...
try:
    ...
    raise SystemExit(f"This script requires camelot-py and PyMuPDF. Install with: pip install camelot-py[pdf] pymupdf")
...
def convert_bbox_to_fitz_rect(bbox: Tuple[float, float, float, float], page_height: float) -> "fitz.Rect":
...
def detect_tables_with_camelot(pdf_path: str, pages: str = "all") -> List[Dict[str, Any]]:
    """
...
def extract_tables_to_images(
    ...
    """
    Extract tables from PDF and save as images.
    ...
    # Detect tables
    tables = detect_tables_with_camelot(pdf_path)
    ...
    for table_info in filtered_tables:
        ...
        try:
            ...
            rect = convert_bbox_to_fitz_rect(bbox, page_height)
